...
8. 判断当前节点哈希值，如果大于0，表示是链表，循环遍历查找key；找到则更新，未找到则尾插新节点
9. 如果当前节点类型是红黑树，执行红黑树的插入逻辑
10. 如果插入了新节点且链表长度大于等于树化阈值，就把链表转换成红黑树
11. 把本次操作添加到元素计数器中，并判断是否需要扩容，如果需要则执行扩容逻辑。

### 初始化数组

put 方法中第一次初始化数组的逻辑如下：
...
初始化数组时充分考虑了并发的情况，如果一个线程正在执行初始化操作，其他线程该怎么办？
...
扩容方法非常长，100多行，而且变量名非常简化，比如tab、nexttn、fwd、ln、hn等，无法见文知意。阅读源码的技巧就是，先整体后局部，先理论后细节。不要一上来就陷入细节中不能自拔，有些细枝末节可以跳过。
...
1. 创建一个新数组，容量是原数组的2倍
2. 从后向前遍历原数组，每遍历一个元素，使用`synchronized`锁住当前下标位置节点
3. 如果当前下标节点是链表，按`hash & n`拆分为低位链表和高位链表；如果是红黑树，同理拆分为低位树和高位树
4. 把元素拷贝到新数组后，把原数组下标位置设置成转移节点ForwardingNode
5. 遍历完成后，把新数组赋值给原数组
