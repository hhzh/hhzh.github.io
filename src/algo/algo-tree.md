...
二叉树是数据结构面试的绝对核心。从基础的三种遍历，到 LCA、路径和、之字形层序遍历，几乎每场技术面都会涉及。二叉树题的关键不在于算法有多复杂，而在于**对递归本质的理解**和**边界条件的精准处理**。

读完本文你将掌握：
- **15 类二叉树高频题的解法**：从三种遍历到 BST 验证，覆盖面试所有二叉树题型
- **核心遍历模式**：DFS（前/中/后序）、BFS（层序）、递归与迭代——掌握模式而非死记代码
- **二叉树性质应用**：满二叉树、完全二叉树、BST、平衡树的定义与判断

> **📌 面试场景**：二叉树题在美团、阿里的面试中出现率超过 70%。三种遍历是基础必考题，LCA 和路径和是中高级题。
...
> **💡 核心提示**：递归是二叉树的灵魂。90% 的二叉树题都可以用递归解决——定义好 base case（null 节点），处理好当前节点逻辑，左右子树递归。写出优雅递归的关键是：**相信递归函数的正确性，不要在脑子里展开整个调用栈**。
...
| 题目类型 | 核心模式 | 时间复杂度 | 空间复杂度 | 面试推荐 |
|----------|----------|------------|------------|----------|
| **三种遍历** | 递归/迭代 | O(n) | O(h) | 必考 |
| **层序遍历** | BFS + 队列 | O(n) | O(w) | 必考 |
| **之字形层序** | BFS + 双端队列 | O(n) | O(w) | 高频 |
| **最大深度** | DFS 递归 | O(n) | O(h) | 必考 |
| **路径和** | DFS + 回溯 | O(n) | O(h) | 高频 |
| **LCA (BST)** | BST 性质 | O(h) | O(h) | 高频 |
| **镜像树** | 递归交换 | O(n) | O(h) | 中频 |
| **合并树** | 递归合并 | O(n) | O(h) | 中频 |
| **对称树** | 递归比较 | O(n) | O(h) | 高频 |
| **平衡树** | 自底向上 | O(n) | O(h) | 高频 |
| **BST 验证** | 上下界传递 | O(n) | O(h) | 高频 |
| **完全二叉树** | BFS + 标记 | O(n) | O(w) | 中频 |

### 生产环境避坑指南

> **⚠️ 避坑 1**：BST 验证时不能只比较节点和直接子节点——BST 要求左子树**所有**节点小于根节点，右子树**所有**节点大于根节点。必须传递上下界参数。

> **⚠️ 避坑 2**：递归中序遍历 BST 时，如果节点值可能等于 `Integer.MIN_VALUE`，用 `int prev = Integer.MIN_VALUE` 初始化前驱值会导致误判。应使用 `Integer` 包装类并判 null。

> **⚠️ 避坑 3**：判断平衡二叉树时，如果在每个节点都调用 `getHeight` 方法，时间复杂度会退化到 O(n²)。正确的做法是自底向上递归，在计算高度的同时判断平衡性。

> **⚠️ 避坑 4**：后序遍历的迭代实现是最复杂的——需要用一个 `prev` 指针记录上一次访问的节点，以判断右子树是否已经处理完毕。面试中如果要求手写后序迭代遍历，这是最大的考点。

### 行动清单

1. **背诵模板**：前/中/后序遍历的递归实现——这是所有二叉树题的基础
2. **理解本质**：为什么 BST 的中序遍历是有序序列？这是 BST 定义决定的——左 < 根 < 右
3. **练习题目**：LeetCode 104（最大深度）、LeetCode 98（BST 验证）、LeetCode 236（LCA）、LeetCode 102（层序遍历）
4. **扩展阅读**：推荐阅读《算法（第4版）》第3章二叉搜索树，以及 JDK `TreeMap` 源码中的红黑树实现