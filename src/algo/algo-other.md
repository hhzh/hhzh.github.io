...
算法面试中，除了排序和字符串，还有一类"综合题"——接雨水、连通分量、矩阵查找、LRU 缓存、多线程交替打印。这些题目考察的不是单一数据结构，而是**多种思想的组合运用**。

读完本文你将掌握：
- **6 类高频综合题的解法**：接雨水、省份数量、有序矩阵、菜单树、双栈队列、多线程打印、LRU 缓存
- **多解法权衡分析**：从暴力到最优，理解每种方法的适用边界
- **底层模式识别**：双指针、单调栈、并查集、信号量——何时用哪个

> **📌 面试场景**：接雨水是字节跳动最高频的题目之一，LRU 缓存是美团/阿里必考题，多线程交替打印考察并发基础。这些题不能靠背诵，必须理解原理。
...
> **💡 核心提示**：接雨水的双指针解法是面试最常考的——左右指针向中间靠拢，每次处理较小的一边，空间复杂度 O(1)。这个思路同样适用于"盛最多水的容器"（LeetCode 11）。

```mermaid
flowchart LR
    A[接雨水] --> B{双指针法}
    B --> C[leftMax < rightMax?]
    C -->|是| D[处理left侧<br/>水量=leftMax-height[left]]
    C -->|否| E[处理right侧<br/>水量=rightMax-height[right]]
    D --> F[left++]
    E --> G[right--]
    F --> H[left < right?]
    G --> H
...
| 题目 | 最优解法 | 时间复杂度 | 空间复杂度 | 面试推荐 |
|------|----------|------------|------------|----------|
| **接雨水** | 双指针 | O(n) | O(1) | 必考 |
| **省份数量** | 并查集 | O(n² α(n)) | O(n) | 高频 |
| **有序矩阵** | 右上/左下指针 | O(n+m) | O(1) | 中频 |
| **菜单树** | HashMap 建树 | O(n) | O(n) | 低频 |
| **双栈队列** | 惰性倒换 | O(1) 摊还 | O(n) | 高频 |
| **交替打印** | Semaphore | O(n) | O(1) | 中频 |
| **LRU 缓存** | LinkedHashMap | O(1) | O(n) | 必考 |

### 生产环境避坑指南

> **⚠️ 避坑 1**：并查集如果不做路径压缩（`find` 中的 `parent[i] = find(parent, parent[i])`），最坏情况下会退化成链表，时间复杂度从 O(α(n)) 变成 O(n)。面试时一定要提到这个优化。

> **⚠️ 避坑 2**：LRU 缓存的手动实现中，双向链表的 `removeNode` 方法必须正确处理 `head` 和 `tail` 指针的更新——否则在删除首尾节点时会丢失链表引用。

> **⚠️ 避坑 3**：多线程交替打印中，`ReentrantLock` 的自旋忙等待方案会大量消耗 CPU，实际生产环境应使用 `Condition.await()` 让线程阻塞等待，而不是忙循环。

> **⚠️ 避坑 4**：接雨水的单调栈解法是按"层"计算雨水，而双指针解法是按"列"计算。理解这个区别很重要——有些变种题（如二维接雨水）需要单调栈的思路。

### 行动清单

1. **背诵模板**：LRU 缓存的双向链表+哈希实现——这是面试最高频的手写题之一
2. **理解本质**：接雨水双指针为什么可以只维护两个最大值？因为水的高度由较低的一边决定（木桶原理）
3. **扩展阅读**：推荐阅读《算法（第4版）》第1.5节并查集，以及 JDK `LinkedHashMap` 源码
4. **练习题目**：LeetCode 42（接雨水）、LeetCode 146（LRU 缓存）、LeetCode 547（省份数量）
