...
字符串算法是 LeetCode 中最常出现的题型之一，涵盖回文、子串、括号匹配、大数运算等经典模式。面试中 70% 的字符串题目都可以归结为 **滑动窗口、双指针、动态规划、回溯** 四种核心模式。

读完本文你将掌握：
- **12 类字符串高频题的解法**：从大数相加到最长回文子串，覆盖面试所有字符串题型
- **多解法对比分析**：每种题目提供 2-3 种解法，理解权衡取舍
- **底层模式识别**：学会从问题描述中快速判断该用什么算法模式

> **📌 面试场景**：字符串题在字节跳动、美团的面试中出现频率极高，特别是滑动窗口和括号匹配类题目。理解模式比背诵代码重要 10 倍。
...
> **💡 核心提示**：字符串算法的本质是模式识别。掌握以下 4 种模式，可以解决 80% 的字符串 LeetCode 题目。
...
| 模式 | 特征关键词 | 代表题目 | 时间复杂度 |
|------|-----------|----------|------------|
| **滑动窗口** | 最长、最短、连续 | 无重复最长子串 | O(n) |
| **动态规划** | 最长公共、最优解 | LCS、最长公共子串 | O(mn) |
| **双指针** | 回文、反转、对撞 | 回文数判断、整数反转 | O(n) |
| **回溯/DFS** | 所有组合、生成 | 括号生成 | O(4^n/√n) |
| **状态机** | 解析、转换 | atoi、有效括号 | O(n) |
| **中心扩展** | 回文子串 | 最长回文子串 | O(n²) |

### 生产环境避坑指南

> **⚠️ 避坑 1**：大数相加时不要使用 `Integer.parseInt()` 或 `Long.parseLong()` ——题目明确说明字符串长度可达 100000，远超任何整数类型的表示范围。必须模拟竖式计算过程。

> **⚠️ 避坑 2**：滑动窗口中使用 `HashSet` 会导致 O(2n) 的时间复杂度，如果已知字符集范围（如 ASCII 128），使用 `int[128]` 数组替代可将时间降至 O(n) 且空间降至 O(1)。

> **⚠️ 避坑 3**：斐波那契数列的通项公式法存在浮点精度问题，当 n>70 时 `double` 类型的精度不足以准确表示结果。生产环境中应使用矩阵快速幂或迭代法。

> **⚠️ 避坑 4**：Manacher 算法虽然时间复杂度是 O(n)，但实现复杂且常数因子较大。在实际面试中，除非 n 超过 10^5，否则中心扩展法 O(n²) 通常已经足够。

### 行动清单

1. **背诵模板**：滑动窗口模板（右指针扩展+左指针收缩）——这是字符串题最高频的模式
2. **手写练习**：手写括号匹配的栈解法和最长回文子串的中心扩展法——面试最常考
3. **理解本质**：区分"子串"（连续）和"子序列"（不连续）——这是 DP 题的 key insight
4. **扩展阅读**：推荐阅读《剑指 Offer》字符串相关章节，以及 LeetCode Hot 100 中的字符串题
