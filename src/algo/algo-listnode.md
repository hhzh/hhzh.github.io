...
链表是面试中出现频率最高的数据结构之一。几乎所有链表题都围绕 **指针操作** 展开——反转、合并、删除、检测环。链表题的难点不在于算法复杂度，而在于 **边界条件的处理** 和 **指针指向的准确性**。

读完本文你将掌握：
- **15 类链表高频题的解法**：从反转链表到合并 K 个有序链表，覆盖面试所有链表题型
- **核心操作模式**：快慢指针、哑节点、递归与迭代——掌握模式而非死记代码
- **指针操作陷阱**：链表断开、空指针异常、奇偶长度处理

> **📌 面试场景**：链表题在阿里、字节的技术面中出现率超过 60%。反转链表和检测环是必考基础题，合并 K 个链表和 K 个一组翻转是中高级题。
...
> **💡 核心提示**：快慢指针是链表题最强大的工具。检测环、找中点、找倒数第 K 个节点、判断回文——全部依赖快慢指针。掌握这个模式，可以解决 50% 以上的链表题。
...
|----------|----------|------------|------------|----------|
| **反转链表** | 三指针迭代 | O(n) | O(1) | 必考 |
| **区间反转** | 哑节点+三指针 | O(n) | O(1) | 高频 |
| **K个一组反转** | 分组+反转+连接 | O(n) | O(1) | 高频 |
| **删除倒数第N** | 快慢指针 | O(n) | O(1) | 高频 |
| **检测环** | 快慢指针 | O(n) | O(1) | 必考 |
| **回文链表** | 快慢+反转后半段 | O(n) | O(1) | 高频 |
| **合并两个链表** | 归并思想 | O(n+m) | O(1) | 必考 |
| **合并K个链表** | 优先队列/分治 | O(n log k) | O(k)/O(log k) | 高频 |

### 生产环境避坑指南

> **⚠️ 避坑 1**：链表操作中最常见的错误是指针丢失——在 `first.next = second.next` 之后，`first.next` 的原始值就丢失了。务必先保存后续引用再修改指针。

> **⚠️ 避坑 2**：快慢指针检测环时，快指针每次走两步，慢指针走一步。如果快指针初始化为 `head.next`，需要在循环前检查 `head` 和 `head.next` 是否为空。

> **⚠️ 避坑 3**：反转链表后一定要处理尾节点的 `next` 指向 null——否则原链表的尾部会形成环。

> **⚠️ 避坑 4**：合并 K 个链表时，使用优先队列的时间复杂度是 O(n log k)，但空间复杂度是 O(k)（存储 k 个堆元素）。分治法的空间复杂度只有 O(log k)（递归栈）。

### 行动清单

1. **背诵模板**：反转链表的三指针迭代法——这是所有反转题的基础
2. **理解本质**：快慢指针为什么能检测环？因为快指针相对于慢指针的"相对速度"是每轮一步，一定会追上
3. **练习题目**：LeetCode 206（反转链表）、LeetCode 141（检测环）、LeetCode 23（合并 K 个链表）、LeetCode 25（K 个一组反转）
4. **扩展阅读**：推荐阅读《剑指 Offer》链表相关章节，以及 JDK `LinkedList` 源码中的双向链表实现