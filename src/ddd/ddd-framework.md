...
你是否经历过这样的场景：一个电商系统的"订单"概念在用户服务里表示用户行为，在支付服务里表示交易流水，在物流服务里表示配送任务——同一个词在三个服务中有三种完全不同的含义？当业务逻辑像野草一样在代码库的每个角落蔓延时，修改一个"取消订单"的需求，竟然需要排查十几处散落的 `if-else` 分支？

DDD（领域驱动设计）正是解决这种复杂性的系统方法论。读完本文，你将掌握：

- **战略设计**：如何用限界上下文为微服务划定清晰边界
- **战术设计**：如何用聚合根守护数据一致性，用值对象消除空指针灾难
- **上下文映射**：如何选择防腐层、开放主机服务等模式解耦服务间依赖

本文从真实业务痛点出发，带你理解 DDD 如何通过"通用语言"让代码与业务同频共振——这正是架构师面试中区分"会写代码"和"会设计系统"的分水岭。
...
> **💡 核心提示**：DDD 不是技术框架，而是一种思维方式。它回答的是"如何组织代码才能让系统随业务一起演进"这个根本问题。
...
* **比喻：** 团队成员之间约定的一套"行话"，大家说"行话"就知道是指什么，并且在任何沟通和代码中都用这套"行话"。
...
### 战术设计 (Tactical Design) - 代码实现

战术设计关注如何在代码层面构建领域模型，将战略设计中划分的限界上下文内的通用语言和业务规则转化为具体的软件构件。

> **💡 核心提示**：战术设计中的所有构件（实体、值对象、聚合、领域服务等）都必须归属于某一个限界上下文。脱离限界上下文讨论这些概念是没有意义的。
...
classDiagram
    ...
    Domain --> BoundedContext : "包含"
    BoundedContext --> AggregateRoot : "包含多个"
    AggregateRoot --> Entity : "管理"
    AggregateRoot --> ValueObject : "管理"
    AggregateRoot --> DomainService : "使用"
    AggregateRoot --> DomainEvent : "发布"
    Repository "1" o-- "*" AggregateRoot : "持久化"
```
...
```mermaid
flowchart TD
    Client[客户端/UI] --> AppService[应用服务层]
    AppService -->|协调| DomainModel[领域模型层]
    DomainModel -->|包含| AR[聚合根]
    DomainModel -->|包含| ES[领域服务]
    DomainModel -->|包含| DE[领域事件]
    AppService -->|调用| Repo[仓库接口]
    Repo -.实现.-> RepoImpl[仓库实现]
    RepoImpl --> Infra[基础设施层]
    Infra --> DB[(数据库)]
    Infra --> MQ[(消息队列)]
    DE -->|发布到| MQ
    AR -->|触发| DE
    ...
    subgraph 依赖方向
        AppService -.依赖.-> DomainModel
        AppService -.依赖.-> Repo
        DomainModel -.依赖.-> Infra接口
    end
```
...
> **💡 核心提示**：注意 DDD 分层中的依赖倒置——领域层定义仓库接口，基础设施层实现它。这确保了领域模型不依赖任何技术框架，保持纯粹的业务表达。
...
| 概念 | 关注点 | 分类/关系 | 对架构的影响 |
| :--- | :--- | :--- | :--- |
| **核心子域** | 业务竞争力 | 核心/支撑/通用 | 投入最多资源建模，需要领域专家深度参与 |
| **支撑子域** | 业务支撑能力 | 核心/支撑/通用 | 可以简化实现，但不影响核心流程即可 |
| **通用子域** | 通用技术能力 | 核心/支撑/通用 | 优先使用第三方方案，避免重复造轮子 |
| **限界上下文** | 模型语义边界 | 独立边界 | 微服务划分的直接依据 |
| **上下文映射** | 上下文间关系 | ACL/OHS/Shared Kernel 等 | 决定服务间集成方式和耦合程度 |
...
> **💡 核心提示**：不要在所有场景中使用 DDD。对于 CRUD 占主导、业务规则简单的系统（如后台管理系统），过度使用 DDD 会带来不必要的复杂度。DDD 最适合业务规则复杂、变更频繁的核心业务域。
...
* **请解释一下 DDD 中的聚合 (Aggregate) 和聚合根 (Aggregate Root)。它们的作用是什么？为什么所有对聚合内对象的数据修改都必须通过聚合根？** (**核心！** 聚合：实体值对象集群，一致性边界。聚合根：聚合唯一入口实体。为什么通过聚合根：保证聚合内数据的一致性)
...
| 坑点 | 症状 | 解决方案 |
| :--- | :--- | :--- |
| **贫血模型** | 实体类只有 getter/setter，业务逻辑散落在 Service 层 | 将行为内聚到领域对象，使用充血模型 |
| **聚合过大** | 一个聚合根管理过多实体，导致事务过重和锁竞争 | 按一致性边界拆分聚合，参考"小聚合"原则 |
| **聚合过小** | 业务操作需要跨多个聚合完成，事务无法保证 | 通过领域事件实现最终一致性 |
| **滥用领域服务** | 所有业务逻辑都放在 Domain Service 中，实体变成数据容器 | 优先将行为放入实体/值对象，领域服务仅处理跨实体逻辑 |
| **上下文边界模糊** | 不同上下文共用同一个数据库表，导致模型污染 | 每个限界上下文使用独立的数据库，通过防腐层集成 |
| **动态 Mapping 失控** | 值对象没有实现不可变性，并发场景出现脏数据 | 确保值对象的所有字段在构造后不可修改 |
| **仓库越界** | 仓库方法返回 DTO 而非聚合根，绕过了聚合的一致性约束 | 仓库只返回聚合根，复杂查询使用 CQRS 模式分离读写 |
| **事件风暴陷阱** | 领域事件过多过细，导致事件驱动变成"事件泛滥" | 只发布有业务意义的状态变更事件，避免技术事件混入领域层 |

### 行动清单

1. **检查点**：审查当前项目的实体类，是否存在大量只有 getter/setter 的贫血模型？将业务行为迁移到实体中。
2. **优化建议**：为值对象添加不可变性约束——所有字段通过构造器初始化，不提供 setter，重写 `equals()` 和 `hashCode()`。
3. **架构审查**：检查服务间是否存在共享数据库？如果是，考虑按限界上下文拆分数据库，引入防腐层。
4. **扩展阅读**：推荐阅读 Eric Evans 的《领域驱动设计：软件核心复杂性应对之道》和 Vaughn Vernon 的《实现领域驱动设计》（蓝皮书）。
5. **实践建议**：从一个小核心子域开始，组织一次事件风暴（Event Storming）工作坊，邀请业务专家和技术团队共同参与。
