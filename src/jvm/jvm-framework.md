...
本文将从底层内存布局出发，全面拆解 JVM 运行时数据区的 6 个核心区域，让你不仅知道「是什么」，更理解「为什么」。读完之后，你将能够：

- 准确说出 JVM 内存区域的划分、线程私有/共享属性、各自可能发生的内存错误
- 理解 Java 8 中 PermGen 到 Metaspace 的变化及其背后的原因
- 快速定位 OOM、StackOverflowError 的根本原因
- 从容应对面试中关于 JVM 内存模型的所有高频问题
...
**核心问题：** 既然 JVM 帮我们管理内存，为什么还会频繁发生 OOM？

> **💡 核心提示**
> 自动内存管理不等于「无需关心内存」。GC 只能回收不再被引用的对象，但如果代码中存在无意或有意保留的对象引用（比如未被清理的静态集合、未关闭的连接），GC 也无能为力。理解内存区域的划分，是诊断一切内存问题的起点。
...
flowchart TB
    subgraph JVM["JVM 运行时数据区"]
        subgraph Shared["线程共享区域"]
            Heap["Java 堆<br/>- 对象实例/数组<br/>- GC 主要区域<br/>- OOM: Java heap space"]
            Method["方法区（Metaspace）<br/>- 类元信息/静态变量<br/>- Java 8+: 本地内存<br/>- OOM: Metaspace"]
            RCP["运行时常量池<br/>- 字面量/符号引用<br/>- Java 7+: String Pool 在堆中"]
        end
        subgraph Private["线程私有区域"]
            PC["程序计数器<br/>- 当前指令地址<br/>- 唯一不会 OOM 的区域"]
            JVS["Java 虚拟机栈<br/>- 栈帧（方法调用）<br/>- StackOverflowError"]
            NMS["本地方法栈<br/>- native 方法调用<br/>- StackOverflowError"]
        end
    end
...
- **面试要点：** 线程私有性、存储内容（栈帧构成）、StackOverflowError 的原因。
...
- **面试要点：** 线程共享、存放对象/数组、GC 主要区域、OOM: Java heap space。

```mermaid
flowchart LR
    subgraph Heap["Java 堆（分代模型）"]
        subgraph Young["新生代"]
            Eden["Eden 区<br/>新对象诞生地"]
            S0["Survivor 0（S0）"]
            S1["Survivor 1（S1）"]
        end
        Old["老年代<br/>长期存活对象"]
        Meta["元空间（Metaspace）<br/>类元信息（本地内存）"]
    end
    ...
    S0 -->|下一次 GC 存活| S1
    S1 -->|多次 GC 后晋升| Old
    Old -->|空间不足| FullGC[Full GC]
    FullGC -->|仍不足| OOM[OOM: Java heap space]
```
...
> **💡 核心提示**
> **PermGen 到 Metaspace 是面试必考题。** Java 8 之前方法区实现在堆内的永久代，有固定大小限制；Java 8 及以后改为元空间，存储在本地内存中，默认无上限。这一变化从根本上解决了 PermGen 溢出问题，同时提升了 GC 效率和内存利用率。

**演进：PermGen（永久代）vs Metaspace（元空间）**
...
- **面试要点：** 线程共享、存储内容（类元信息、静态变量等）、OOM 类型（`PermGen space` vs `Metaspace`）、**PermGen 到 Metaspace 的变化、原因、存储位置、默认大小**。
...
- **面试要点：** 作用、位置、String Pool 的位置变化及其原因。
...
| 方法区（Metaspace） | 共享 | 类元信息 | 类结构信息、运行时常量池、静态变量、JIT 代码 | `OutOfMemoryError: Metaspace` |
| 运行时常量池 | 共享 | 字面量和符号引用 | 字符串常量、类/方法/字段引用等 | `OutOfMemoryError: Runtime constant pool` |

### 常见版本差异对比

| 对比项 | Java 7 及之前 | Java 8 及以后 |
|-------|-------------|-------------|
| 方法区实现 | PermGen（永久代） | Metaspace（元空间） |
| 存储位置 | JVM 堆内 | 本地内存（Native Memory） |
| 大小限制 | `-XX:MaxPermSize`（固定） | 默认无上限，可用 `-XX:MaxMetaspaceSize` 限制 |
| String Pool 位置 | PermGen 中 | Java 堆中 |
...
- **请列举 JVM 运行时数据区。哪些是线程私有的？哪些是线程共享的？** 列出 5+1 个区域并正确区分私有/共享。
...
- **方法区的作用是什么？它存储了哪些内容？可能发生什么错误？** 作用：类元信息。内容：类结构、常量池、静态变量、JIT 代码。错误：OOM: PermGen/Metaspace。
- **请解释 Java 8 中方法区的变化。PermGen 和 Metaspace 有什么区别？** 必考题。详细对比：PermGen 在堆内固定大小 vs Metaspace 在本地内存默认无上限。变化原因：解决 PermGen 溢出、GC 复杂性等。
...
- **请简述常见的内存溢出错误类型及其发生原因。** 回顾各个区域的 OOM 类型及原因。
- **JVM 栈溢出（StackOverflowError）通常是什么原因导致的？** 方法递归调用层数过深。
- **Java 堆为什么要分代？** 与 GC 算法相关，根据对象生命周期采用不同回收策略，提高 GC 效率。

### 生产环境避坑指南

| 陷阱 | 现象 | 根因 | 解决方案 |
|------|------|------|---------|
| 未设置 -Xms/-Xmx | 启动后堆频繁扩缩容，性能抖动 | JVM 默认动态调整堆大小 | `-Xms` 和 `-Xmx` 设为相同值 |
| -Xss 设置过大 | 线程创建数量受限，无法启动更多线程 | 每线程栈内存 = -Xss 值 | 根据方法调用深度合理设置（通常 256K-1M） |
| Metaspace 无上限 | 类加载泄漏时本地内存耗尽 | 未设置 MaxMetaspaceSize | 生产环境设置 `-XX:MaxMetaspaceSize=256m` |
| 静态集合持续添加 | 运行一段时间后 OOM: Heap space | 静态变量持有对象引用，GC 无法回收 | 使用弱引用或定期清理 |
| 大量字符串 intern | OOM: Heap space | String Pool 在堆中，intern 过多撑爆堆 | 控制 intern 使用量，或增大堆内存 |

### 行动清单

- [ ] 确认你的项目使用的 JDK 版本，理解方法区在该版本的实现（PermGen 还是 Metaspace）
- [ ] 检查生产环境 JVM 启动参数，确保 `-Xms` 和 `-Xmx` 设置为相同值
- [ ] 为 Metaspace 设置合理的上限：`-XX:MaxMetaspaceSize=256m`
- [ ] 开启 OOM 自动 Dump：`-XX:+HeapDumpOnOutOfMemoryError`
- [ ] 排查代码中是否有静态集合（`static List/Map`）持续添加元素的情况
- [ ] 理解 Java 堆的分代模型（Eden、S0、S1、Old），为后续 GC 调优打基础
- [ ] 复习栈帧的四个组成部分，理解方法调用与内存的关系
