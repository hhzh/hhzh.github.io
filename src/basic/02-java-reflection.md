...
读完本文你将掌握：Class 对象与类元数据的关系、`Method.invoke` 的调用链路与膨胀阈值原理、动态代理的字节码生成机制，以及 Java 9+ 模块化对反射的限制。这些知识是理解 Spring AOP、序列化框架、ORM 工具底层原理的重要基础。
...
在 JVM 中，每个加载的类都会在堆中生成一个 `Class` 对象，作为访问该类元数据的入口。真正的方法字节码、常量池、字段布局等信息存放在方法区的实现中；在 HotSpot JDK 8 之后，这部分主要位于元空间（Metaspace）。可以把 `Class` 对象理解为堆上的“门牌”，类元数据则是门牌背后的实际内容。
...
| 18+     | 9,800            | 98,760            | 8,950                 |

**结论**：
- 反射调用的性能损耗主要来自**参数装箱**和**访问检查**。
- JDK 18 通过 JEP 416 重新实现核心反射，改为基于 MethodHandle，减少了传统反射调用路径中的维护成本。
