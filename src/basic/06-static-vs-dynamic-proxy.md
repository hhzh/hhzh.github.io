...
Spring AOP 为什么有时走 JDK 代理，有时走 CGLIB？同样是给方法加事务、日志或权限校验，代理对象到底是怎么把调用转交给目标对象的？理解这条链路，才能解释 `$Proxy0`、FastClass 和 `@Transactional` 失效这类常见问题。
...
掌握这些底层原理，不仅能应对面试中的代理问题，也能在生产环境中更快定位 AOP 失效、代理链过深和元空间增长等问题。

## 代理模式的"双重人格"：隔离与控制

代理模式的核心是**控制访问**并**扩展功能**。通过代理对象间接操作目标对象，既能实现业务逻辑隔离（如权限校验），又能无缝添加日志、事务等扩展功能。
