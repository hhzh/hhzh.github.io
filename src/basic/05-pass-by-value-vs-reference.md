...
Java 到底是值传递还是引用传递？很多人被“对象传进去以后能被修改”这个现象绕住了：既然方法里能改对象内容，为什么还说 Java 只有值传递？答案在 JVM 栈帧里——传进去的不是对象本身，而是引用值的副本。
...
掌握这些底层原理，不仅能把面试中的概念题说清楚，也能在业务代码里更谨慎地处理副作用、装箱和递归栈深度。
...
从 JVM 的栈帧结构回到语言设计层面，Java 的值传递选择并非唯一解。C++ 提供了真正的引用传递，两者在语义上有本质差异。
...
理解了参数传递的性能陷阱后，一个自然的优化方向是——如何设计更高效的数据传输对象？Java 14 引入的 Record 类给出了答案。

## Record 类优化 DTO 传递效率

Java 14 引入的 `Record` 类通过不可变语义和自动生成方法，让 DTO 更简洁：
...
**优势**：
1. **样板代码更少**：编译器自动生成构造方法、访问器、`equals`、`hashCode` 和 `toString`。
2. **语义更清晰**：Record 默认是浅不可变数据载体，更适合表达只读 DTO。
3. **线程安全**：字段 final 修饰，天然避免并发修改问题。

Record 的主要价值不是“天然更快”，而是让 DTO 的不可变语义更明确。具体序列化性能取决于 JSON 框架、反射缓存和字段数量，建议用 JMH 或真实压测确认。
...
Java 参数传递的本质只有一个——值传递。无论是基本类型的值复制，还是引用类型的地址复制，传递的始终是副本。理解这一点，就能解开"修改对象内容"与"修改引用指向"之间的困惑，也能在面试中清晰区分 Java 与 C++ 引用传递的根本差异。在工程实践中，这意味着：传递引用类型参数时务必注意副作用，高频路径优先使用基本类型，递归场景警惕栈溢出。
...
4. **谨慎调优 Integer 缓存**：只有在确认高频装箱值集中落在某个范围内时，才考虑通过 `-XX:AutoBoxCacheMax=1000` 扩大缓存；大多数业务优先改用基本类型更直接。
