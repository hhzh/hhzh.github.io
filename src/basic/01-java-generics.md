...
PECS 原则解决了通配符的使用问题，但类型擦除带来的另一个挑战是——运行时如何获取泛型信息？类型擦除导致运行时泛型信息丢失，但以下场景例外：
...
泛型的类型推断不仅体现在方法调用中，Java 10 的 var 关键字也与之有微妙的交互。
...
理解了 Java 泛型的全貌后，一个自然的问题是——其他语言是怎么做的？
...
掌握 Java 泛型的类型擦除机制、PECS 原则和桥方法生成，不仅能让你在面试中从容应对，更能在工程实践中避开那些隐蔽的类型安全陷阱。泛型的设计是 Java 在兼容性与表达能力之间的一次妥协——理解这种妥协，才是真正理解 Java 类型系统的关键。
