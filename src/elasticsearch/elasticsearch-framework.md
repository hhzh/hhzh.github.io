...
classDiagram
    ...
    Cluster --> MasterNode : "选举一个"
    Cluster --> DataNode : "包含多个"
    Cluster --> CoordinatingNode : "任一节点可充当"
    Cluster --> Index : "包含多个"
    Index --> Shard : "划分为多个"
    Shard --> DataNode : "分配在"
```
...
flowchart TD
    subgraph 索引流程
        A[客户端发送索引请求] --> B[Coordinating Node 接收]
        B --> C[计算路由: hash(id) % shard_count]
        C --> D[转发到 Primary Shard 所在节点]
        D --> E[Primary Shard 写入 Translog]
        E --> F[写入内存 Buffer]
        F --> G[Refresh 到 Lucene Segment]
        G --> H[并行复制到 Replica Shards]
        H --> I[Replica 确认]
        I --> J[Primary 确认给 Coordinating Node]
        J --> K[返回成功给客户端]
    end

    subgraph 搜索流程
        L[客户端发送搜索请求] --> M[Coordinating Node 接收]
        M --> N[Query Phase: 分发到所有相关 Shard]
        N --> O[每个 Shard 执行本地搜索]
        O --> P[返回匹配的 Document ID + 分数]
        P --> Q[Coordinating Node 归集排序]
        Q --> R[Fetch Phase: 获取完整文档内容]
        R --> S[返回最终结果给客户端]
    end
```
